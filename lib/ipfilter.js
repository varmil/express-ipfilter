...
module.exports = function ipfilter(ips, opts) {
  ...
  const testRange = (req, ip, constraint, mode) => {
    const filteredSet = _.filter(getIps(req), constraint => {
  ...
  const testIp = function(req, ip, mode) {
    ...
    if (typeof constraint === 'object') {
      return testRange(req, ip, constraint, mode)
    }
  }

  const matchClientIp = (ip, req) => {
    const mode = settings.mode.toLowerCase()

    const result = _.invokeMap(getIps(req), testIp, req, ip, mode)
